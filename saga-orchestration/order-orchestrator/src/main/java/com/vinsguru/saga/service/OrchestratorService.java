...
public class OrchestratorService {
    ...
    public Mono<OrchestratorResponseDTO>  orderProduct(final OrchestratorRequestDTO requestDTO){
        ...
        return Flux.fromStream(() -> orderWorkflow.getSteps().stream())
                ...
                .handle(((aBoolean, synchronousSink) -> {
                     log.info("outcome of process ::: {}",aBoolean);
                    if(aBoolean)
                        synchronousSink.next(true);
                    else
                        synchronousSink.error(new WorkflowException("create order failed!"));
                }))
                .then(Mono.fromCallable(() -> {log.info("updating response dto with completed status ");
    ...
    private Workflow getOrderWorkflow(OrchestratorRequestDTO requestDTO){
        log.info("** Constructing workflow with steps **");
        ...
        log.info("** Constructing paymentStep with steps ** {}",paymentStep);
        log.info("** Constructing inventoryStep with steps ** {}",inventoryStep);
